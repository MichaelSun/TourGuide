...
public class UserHelper {
	...
	public void login(final String mobileNum, final String password,
		...
		CustomThreadPool.asyncWork(new Runnable() {
			...
			public void run() {
				try {
					...
					LoginResponse response = InternetUtils.request(mContext,
							request);
					if (response != null) {
						if (response.userId > 0
								&& response.userPassport != null
								&& !TextUtils
										.isEmpty(response.userPassport.ticket)
								&& !TextUtils
										.isEmpty(response.userPassport.userSecretKey)) {
							SettingManager.getInstance().setUserId(
									response.userId);
							SettingManager.getInstance().setUserPhoneNum(
									mobileNum);
							SettingManager.getInstance().setUserName(
									response.userName);
							SettingManager.getInstance().setUserGender(
									response.gender);
							SettingManager.getInstance().setUserType(
									response.userType);
							SettingManager.getInstance().setUserHeader(
									response.headUrl);
							SettingManager.getInstance().setTicket(
									response.userPassport.ticket);
							SettingManager.getInstance().setSecretKey(
									response.userPassport.userSecretKey);
	...
	public void register(final String mobileNum, final String password,
			final String name, final String verifyCode, final int gender,
			OnRegisterFinishListener listener) {
		...
		CustomThreadPool.asyncWork(new Runnable() {
			...
			public void run() {
				try {
					RegisterRequest request = new RegisterRequest(mobileNum,
							password, name, verifyCode, gender);
					RegisterResponse response = InternetUtils.request(mContext,
							request);
					if (response != null) {
						if (response.userId > 0
								&& !TextUtils.isEmpty(response.ticket)
	...
	public void getVerifyCode(final String mobile,
			OnGetVerifyCodeFinishListener listener) {
			...
			public void run() {
				try {
					GetVerifyCodeRequest request = new GetVerifyCodeRequest(
							mobile);
					GetVerifyCodeResponse response = InternetUtils.request(
							mContext, request);
					if (response != null) {
						if (response.result == 0) {
							if (mOnGetVerifyCodeFinishListener != null) {
								mOnGetVerifyCodeFinishListener
										.onGetVerifyCodeFinish(SUCCESS);
							}
						} else {
							if (mOnGetVerifyCodeFinishListener != null) {
								mOnGetVerifyCodeFinishListener
										.onGetVerifyCodeFinish(FAILED);
							}
						}
						return;
					}
				...
				if (mOnGetVerifyCodeFinishListener != null) {
					mOnGetVerifyCodeFinishListener
							.onGetVerifyCodeFinish(NETWORK_ERROR);
				}
			}
		});
	}
	...
	public void applyForGuide(final String goodAtScenic, final long birthday,
			final int beGuideYear, final String guideCardUrl,
			final String guideCardId, final String location, final int city,
			...
			public void run() {
				try {
					...
					if (!TextUtils.isEmpty(guideCardUrl)) {
						UploadResourceRequest urRequest = new UploadResourceRequest(
								guideCardUrl, "jpg");
						UploadResourceResponse urResponse = InternetUtils
								.request(mContext, urRequest);
						if (urResponse != null) {
							url = urResponse.url;
						}
					}
					...
					if (TextUtils.isEmpty(url)) {
						if (mOnApplyForGuideFinishListener != null) {
							mOnApplyForGuideFinishListener
									.onApplyForGuideFinish(FAILED);
						}
						return;
					}

					ApplyForGuideRequest request = new ApplyForGuideRequest(
							goodAtScenic, birthday, beGuideYear, url,
							guideCardId, location, city, agency);
					ApplyForGuideResponse response = InternetUtils.request(
							mContext, request);
					if (response != null && response.result == 0) {
						if (response.result == 0) {
							SettingManager.getInstance().setUserType(
									Tourist.USER_TYPE_TOURGUIDE);
							if (mOnApplyForGuideFinishListener != null) {
								mOnApplyForGuideFinishListener
										.onApplyForGuideFinish(SUCCESS);
							}
						} else {
							if (mOnApplyForGuideFinishListener != null) {
								mOnApplyForGuideFinishListener
				...
				if (mOnApplyForGuideFinishListener != null) {
					mOnApplyForGuideFinishListener
							.onApplyForGuideFinish(NETWORK_ERROR);
				}
			}
		});
	}
	...
	public void getUserInfo(final int userId,
			OnGetUserInfoFinishListener listener) {
		...
		CustomThreadPool.asyncWork(new Runnable() {
			...
			public void run() {
				try {
					...
					GetUserInfoResponse response = InternetUtils.request(
							mContext, request);
					if (response != null && response.tourist.getUserId() > 0) {
						if (mOnGetUserInfoFinishListener != null) {
							mOnGetUserInfoFinishListener
									.onGetUserInfoSuccess(response.tourist);
						}
						return;
					}
				} catch (NetWorkException e) {
	...
	public interface OnGetNearByGuideListener {
		...
		public void onGetMoreNearByGuideFinish(int result,
				List<TourGuide> guides);
	}

	public void getNearByGuide(final String location, final double dist,
			final int start, final int rows, OnGetNearByGuideListener listener) {
		...
		CustomThreadPool.asyncWork(new Runnable() {
			...
			public void run() {
				try {
					GetNearByGuideRequest request = new GetNearByGuideRequest(
							location, dist, start, rows);
					GetNearByGuideResponse response = InternetUtils.request(
							mContext, request);
					if (response != null) {
						List<TourGuide> guides = response.guides;
						// 去掉自己
						if (guides != null
								&& SettingManager.getInstance().getUserId() > 0) {
							for (int i = 0; i < guides.size(); i++) {
								TourGuide guide = guides.get(i);
								if (guide.getUserId() == SettingManager
						...
						if (mGetNearByGuideListener != null) {
							if (start > 0) {
								mGetNearByGuideListener
										.onGetMoreNearByGuideFinish(SUCCESS,
												guides);
							} else {
								mGetNearByGuideListener.onGetNearByGuideFinish(
										SUCCESS, guides);
							}
				...
				if (mGetNearByGuideListener != null) {
					if (start > 0) {
						mGetNearByGuideListener.onGetMoreNearByGuideFinish(
								NETWORK_ERROR, null);
					} else {
						mGetNearByGuideListener.onGetNearByGuideFinish(
								NETWORK_ERROR, null);
	...
	public void searchGuide(final int city, final int gender,
			final String scenic, final int start, final int rows,
		...
		CustomThreadPool.asyncWork(new Runnable() {
			...
			public void run() {
				try {
					SearchGuideRequest request = new SearchGuideRequest(city,
							gender, scenic, start, rows);
					SearchGuideResponse response = InternetUtils.request(
							mContext, request);
					if (response != null) {
						List<TourGuide> guides = response.guides;
						// 去掉自己
						...
						if (mOnSearchGuideListener != null) {
							if (start > 0) {
								mOnSearchGuideListener.onSearchMoreGuide(
										SUCCESS, guides);
							} else {
								mOnSearchGuideListener.onSearchGuide(SUCCESS,
										guides);
							}
						}
						return;
				...
				if (mOnSearchGuideListener != null) {
					if (start > 0) {
						mOnSearchGuideListener.onSearchMoreGuide(NETWORK_ERROR,
								null);
					} else {
						mOnSearchGuideListener.onSearchGuide(NETWORK_ERROR,
								null);
					}
				}
			}
			...
			public void run() {
				try {
					...
					if (!TextUtils.isEmpty(url)) {
						...
						ChangeHeadResponse response = InternetUtils.request(
								mContext, request);
						if (response != null) {
							if (response.result == 0) {
	...
	public void changeLocation(final String location,
			OnChangeLocationListener listener) {
		...
		CustomThreadPool.asyncWork(new Runnable() {
			...
			public void run() {
				try {
					...
					ChangeLocationResponse response = InternetUtils.request(
							mContext, request);
					if (response != null) {
						if (response.result == 0) {
							if (mOnChangeLocationListener != null) {
								mOnChangeLocationListener
										.onChangeLocation(SUCCESS);
							}
						} else {
							if (mOnChangeLocationListener != null) {
								mOnChangeLocationListener
										.onChangeLocation(FAILED);
							}
						}
						return;
					}
	...
	public void changeUserInfo(final String name, final int gender,
			final String headPath, OnChangeUserInfoListener listener) {
				...
				try {
					...
					if (!TextUtils.isEmpty(url)) {
						ChangeUserInfoRequest request = new ChangeUserInfoRequest(
								name, gender, url);
						ChangeUserInfoResponse response = InternetUtils
								.request(mContext, request);
						if (response != null) {
							if (response.result == 0) {
								SettingManager.getInstance().setUserName(name);
								SettingManager.getInstance().setUserGender(
								...
								if (mOnChangeUserInfoListener != null) {
									mOnChangeUserInfoListener
											.onChangeUserInfo(SUCCESS);
								}
							} else {
								if (mOnChangeUserInfoListener != null) {
									mOnChangeUserInfoListener
											.onChangeUserInfo(FAILED);
								}
							}
							return;
						}
			...
			public void run() {
				try {
					ChangePasswordRequest request = new ChangePasswordRequest(
							oldPwd, newPwd);
					ChangePasswordResponse response = InternetUtils.request(
							mContext, request);
					if (response != null) {
						if (response.result == 0) {
