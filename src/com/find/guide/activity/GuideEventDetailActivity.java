...
public class GuideEventDetailActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			if (getIntent() != null) {
				mGuideEvent = (GuideEvent) getIntent().getSerializableExtra(
						INTENT_EXTRA_GUIDE_EVENT_OBJ);
			}
		} else {
			mGuideEvent = (GuideEvent) savedInstanceState
					.getSerializable(INTENT_EXTRA_GUIDE_EVENT_OBJ);
		}
	...
	private void initUI() {
		...
		if (mGuideEvent.getEventStatus() == GuideEvent.EVENT_STATUS_SATISFACTION) {
			mEndTimeLayout.setBackgroundResource(R.drawable.bg_middle);
			mSatisfactionLayout.setVisibility(View.VISIBLE);
			String satisfaction = "";
			if (mGuideEvent.getSatisfaction() == GuideEvent.SATISFACTION_GOOD) {
				satisfaction = "(满意) ";
	...
	private void accept() {
		TipsDialog.getInstance().show(this, R.drawable.tips_loading,
				R.string.accepting_invite, true, false);
		mGuideHelper.accept(mGuideEvent.getEventId(), mGuideEvent.getUserId(),
				mOnAcceptedListener);
	}

	private void refuse() {
		TipsDialog.getInstance().show(this, R.drawable.tips_loading,
				R.string.refusing_invite, true, false);
		mGuideHelper.refuse(mGuideEvent.getEventId(), mGuideEvent.getUserId(),
				mOnRefusedListener);
	}
