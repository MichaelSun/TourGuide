...
public class CropImageActivity extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            mSelectImageMode = getIntent().getIntExtra(EXTRA_SELECT_IMAGE_MODE, 1);
        }
    ...
    private void initUI() {
        ...
        RelativeLayout.LayoutParams topLp = (LayoutParams) topView.getLayoutParams();
        topLp.topMargin = marginTop;
        RelativeLayout.LayoutParams bottomLp = (LayoutParams) bottomView.getLayoutParams();
        bottomLp.topMargin = marginTop + screenWidth;
        RelativeLayout.LayoutParams leftLp = (LayoutParams) leftView.getLayoutParams();
        leftLp.topMargin = marginTop;
        leftLp.height = screenWidth;
        RelativeLayout.LayoutParams rightLp = (LayoutParams) rightView.getLayoutParams();
    ...
    private void selectFromCamera() {
        ...
        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(mSourceImagePath)));
    ...
    private void save() {
        ...
        if (mSourceImagePath != null && mSourceBitmap != null && mSourceBitmap.getWidth() != 0
        ...
        mCropImagePath = cropBitmapWithlib(mSourceImagePath, newRect, getApplicationContext());
    ...
    public String cropBitmapWithlib(String srcPath, Rect cropRect, Context context) {
        if (srcPath == null || cropRect == null || cropRect.width() <= 0 || cropRect.height() <= 0) {
            return null;
        }
        CropAndScale cropAndScale = new CropAndScale(context);
        String dir = DiskManager.tryToFetchCachePathByType(DiskCacheType.BASE) + "header_crop";
        File file = new File(dir);
        file.mkdirs();
        String retPath = new File(file, System.currentTimeMillis() + "upload.jpg").getAbsolutePath();
        int targetWidth = 480;
        ...
        int jResult = cropAndScale.cropJpg(srcPath, retPath, cropRect.left, cropRect.top, cropRect.width(),
                cropRect.height(), targetWidth, targetHeight, rotation);
