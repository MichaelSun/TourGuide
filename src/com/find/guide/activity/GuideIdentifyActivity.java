...
public class GuideIdentifyActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_CROP_IMAGE) {
			if (resultCode == RESULT_OK && data != null) {
				String path = data
						.getStringExtra(CropImageActivity.EXTRA_CROP_IMAGE_PATH);
				if (!TextUtils.isEmpty(path)) {
					mGuideCardPath = path;
					mGuideCardBitmap = ImageUtils
							.loadBitmapWithSizeOrientation(mGuideCardPath);
					if (mGuideCardBitmap != null
							&& !mGuideCardBitmap.isRecycled())
			...
			if (resultCode == RESULT_OK) {
				if (data != null) {
					mCityItem = (CityItem) data
							.getSerializableExtra(SelectCityActivity.INTENT_EXTRA_CITY);
					if (mCityItem != null) {
						mCityTv.setText(mCityItem.getCityName());
					}
				}
		...
		mDialog = new DatePickerDialog(this,
				new DatePickerDialog.OnDateSetListener() {
					...
					public void onDateSet(DatePicker view, int year,
						...
						mBirthdayTimestamp = getTimestamp(year, monthOfYear,
								dayOfMonth);
					}
	...
	private void submitCheck() {
		...
		if (TextUtils.isEmpty(mGuideCardPath)
				|| TextUtils.isEmpty(sBeGuideYear)
				|| TextUtils.isEmpty(guideCardId)
				|| TextUtils.isEmpty(goodAtScenic)
				|| TextUtils.isEmpty(travelAgency)) {
			return;
		}
		...
		TipsDialog.getInstance().show(this, R.drawable.tips_loading,
				"导游认证中...", true, false);
		mUserHelper.applyForGuide(goodAtScenic, mBirthdayTimestamp,
				beGuideYear, mGuideCardPath, guideCardId, AppRuntime.gLocation,
				mCityItem != null ? mCityItem.getCityCode() : 20001,
				travelAgency, mApplyForGuideFinishListener);
	}
			...
			mHandler.post(new Runnable() {
				...
				public void run() {
					...
					if (result == UserHelper.SUCCESS) {
						...
						Toasts.getInstance(TourGuideApplication.getInstance())
								.show("认证失败", Toast.LENGTH_SHORT);
					}
				}
			});
		}
