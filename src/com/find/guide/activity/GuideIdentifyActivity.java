...
public class GuideIdentifyActivity extends BaseActivity implements OnClickListener {
    ...
    private View mBeGuideYearView;
    private TextView mBeGuideYearTv;
    private EditText mTravelAgencyEt;
    ...
    private void initUI() {
        ...
        mBeGuideYearTv = (TextView) findViewById(R.id.guide_identify_be_guide_year_tv);
        mBeGuideYearView = findViewById(R.id.guide_identify_be_guide_year_layout);
        mTravelAgencyEt = (EditText) findViewById(R.id.guide_identify_agency_et);
        ...
        mBeGuideYearView.setOnClickListener(this);
    }
    ...
    public void onDestroy() {
        ...
        if (mYearDialog != null && mYearDialog.isShowing()) {
            mYearDialog.dismiss();
            mYearDialog = null;
        }
    ...
    public void onClick(View v) {
        ...
        case R.id.guide_identify_birthday_layout:
            selectBirthday();
            break;
        case R.id.guide_identify_be_guide_year_layout:
            selectBeGuideYear();
            break;
        }
    }
    ...
    private void selectBeGuideYear() {
        if (mYearDialog != null && mYearDialog.isShowing()) {
            mYearDialog.dismiss();
            mYearDialog = null;
        }
        ...
        numPicker.setMaxValue(year);
        numPicker.setMinValue(1949);
        numPicker.setValue(mBeGuideYear);

        mYearDialog = new AlertDialog.Builder(this).setTitle("选择年份").setView(numPicker)
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        mBeGuideYear = numPicker.getValue();
                        mBeGuideYearTv.setText("" + mBeGuideYear);
                    }
                }).create();
        mYearDialog.show();
    }
    ...
    private void submitCheck() {
        String travelAgency = mTravelAgencyEt.getText().toString();
        ...
        if (TextUtils.isEmpty(mGuideCardPath)) {
            showError("请上传导游证");
        } else if (TextUtils.isEmpty(guideCardId)) {
            showError("请输入导游证号");
        } else if (TextUtils.isEmpty(travelAgency)) {
            showError("请输入所属旅行社");
        } else if (TextUtils.isEmpty(goodAtScenic)) {
            showError("请输入擅长景区");
        } else {
            TipsDialog.getInstance().show(this, R.drawable.tips_loading, "导游认证中...", true, false);
            mUserHelper.applyForGuide(goodAtScenic, mBirthdayTimestamp, mBeGuideYear, mGuideCardPath, guideCardId,
                    AppRuntime.gLocation, mCityItem != null ? mCityItem.getCityCode() : 20001, travelAgency,
