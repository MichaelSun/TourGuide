...
public class RegisterActivity extends BaseActivity implements OnClickListener {
	...
	private void register() {
		...
		if (TextUtils.isEmpty(phone)) {
			...
			mLoginHelper.register(phone, password, name, verifyCode, gender,
					...
					if (result == UserHelper.SUCCESS) {
						PushManager.startWork(getApplicationContext(),
								PushConstants.LOGIN_TYPE_API_KEY, PushUtils
										.getMetaValue(RegisterActivity.this,
												"BAIDU_PUSH_APIKEY"));
						setResult(RESULT_OK);
						finish();
					...
					mHandler.post(new Runnable() {
						...
						public void run() {
							if (mResendSeconds <= 0) {
								...
								mSendVerifyCodeTv
										.setText(R.string.register_send_verify_code);
	...
	private void showError(String errorMsg) {
		TipsDialog.getInstance().show(this, R.drawable.tips_fail, errorMsg,
				true);
	}
