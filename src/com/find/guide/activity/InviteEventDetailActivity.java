...
public class InviteEventDetailActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			if (getIntent() != null) {
				mInviteEvent = (InviteEvent) getIntent().getSerializableExtra(
						INTENT_EXTRA_INVITE_EVENT_OBJ);
			}
		} else {
			mInviteEvent = (InviteEvent) savedInstanceState
					.getSerializable(INTENT_EXTRA_INVITE_EVENT_OBJ);
		}
	...
	private void initUI() {
		...
		if (mInviteEvent.getEventType() == InviteEvent.EVENT_TYPE_BROADCASR
				&& (mInviteEvent.getEventStatus() == InviteEvent.EVENT_STATUS_BOOKING || mInviteEvent
						.getEventStatus() == InviteEvent.EVENT_STATUS_CANCELED)) {
	...
	private void dialGuide() {
		...
		mAlertDialog = new AlertDialog.Builder(this)
				.setMessage(msg)
				.setPositiveButton(R.string.call_dialog_positive,
				...
				.setNegativeButton(R.string.call_dialog_negative,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
									int which) {
	...
	private void cancel() {
		TipsDialog.getInstance().show(this, R.drawable.tips_loading,
				R.string.in_cancel, true, false);
		mInviteHelper.cancelInvite(mInviteEvent.getEventId(),
				mInviteEvent.getGuideId());
	}
				...
				public void run() {
					...
					if (result == InviteHelper.SUCCESS) {
						TipsDialog.getInstance().show(
								InviteEventDetailActivity.this,
								R.drawable.tips_saved, "", true);
	...
	private void setSatisfaction(int satisfaction) {
		if (mInviteEvent != null) {
			...
			TipsDialog.getInstance().show(this, R.drawable.tips_loading,
					R.string.in_statisfaction, true, false);
			mInviteHelper.setSatisfaction(mInviteEvent.getEventId(),
					mInviteEvent.getGuideId(), satisfaction, content,
