...
public class BookingActivity extends BaseActivity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		initUI();
		if (getIntent() != null) {
			mTourGuide = (TourGuide) getIntent().getSerializableExtra(
					INTENT_EXTRA_GUIDE);
		}
		setData();
	...
	private void setData() {
		if (mTourGuide != null) {
			mGuideHeaderIv.setImageURI(new Uri.Builder().path(
					mTourGuide.getHeadUrl()).build());
			mGuideNameTv.setText(mTourGuide.getUserName());
			CityItem cityItem = CityManager.getInstance().getCityById(
					mTourGuide.getCity());
			if (cityItem != null) {
		...
		mStartTimeTv.setText(year + "/"
				+ (month + 1 < 10 ? "0" + (month + 1) : month + 1) + "/"
				+ (day < 10 ? "0" + (day) : day) + " "
				+ (hour < 10 ? "0" + (hour) : hour) + ":"
				+ (min < 10 ? "0" + (min) : min));
		mEndTimeTv.setText(year + "/"
				+ (month + 1 < 10 ? "0" + (month + 1) : month + 1) + "/"
				+ (day < 10 ? "0" + (day) : day) + " "
				+ (hour < 10 ? "0" + (hour) : hour) + ":"
				+ (min < 10 ? "0" + (min) : min));
	...
	private void viewLarge() {
		...
		intent.putExtra(ViewLargeActivity.INTENT_EXTRA_URL,
				mTourGuide.getHeadUrl());
	...
	private void bookingGuide() {
		...
		if (mTourGuide != null) {
			...
			if (TextUtils.isEmpty(destination)
					|| TextUtils.isEmpty(destination.trim())) {
				TipsDialog.getInstance().show(this, R.drawable.tips_fail,
						R.string.booking_destination_hint, true);
				return;
			}
			if (mTourGuide.getUserId() == SettingManager.getInstance()
					.getUserId()) {
				TipsDialog.getInstance().show(this, R.drawable.tips_fail,
						R.string.booking_cannot_booking_yourself, true);
				return;
			}
			TipsDialog.getInstance().show(this, R.drawable.tips_loading,
					R.string.in_booking, true, false);
			mInviteHelper.invite(mTourGuide.getUserId(), destination.trim(),
			...
			mHandler.post(new Runnable() {
				...
				public void run() {
					...
					if (result == InviteHelper.SUCCESS) {
						TipsDialog.getInstance().show(BookingActivity.this,
								R.drawable.tips_saved,
						...
						TipsDialog.getInstance().show(BookingActivity.this,
								R.drawable.tips_fail, R.string.booking_failed,
								true);
					}
				}
			});
	...
	private void showLoginDialog() {
		...
		mAlertDialog = new AlertDialog.Builder(this)
				...
				.setPositiveButton(R.string.login_dialog_positive,
				...
				.setNegativeButton(R.string.login_dialog_negative,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
									int which) {
	...
	private void enterGuideHistory() {
		if (SettingManager.getInstance().getUserId() > 0) {
			...
			intent.putExtra(GuideRecordActivity.EXTRA_INTENT_GUIDE_ID,
					mTourGuide.getUserId());
			startActivity(intent);
		} else {
			showLoginDialog();
		}
	...
	private void selectStartTime() {
		dismissDateTimePicker();
		mDateTimePickerDialog = new DateTimePickerDialog(this,
