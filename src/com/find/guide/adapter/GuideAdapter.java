	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(mActivity).inflate(
					R.layout.guide_list_item, null);
			holder = new ViewHolder();
			holder.content = convertView.findViewById(R.id.content);
			holder.header = (WebImageView) convertView
		...
		holder.header.setImageURI(new Uri.Builder().path(guide.getHeadUrl())
				.build());

		double[] latlng1 = parseLocation(guide.getLocation());
		double[] latlng2 = parseLocation(AppRuntime.gLocation);
		double distance = 0.0;
		if (latlng1 != null && latlng2 != null && latlng1.length == 2
				&& latlng2.length == 2) {
			GeoPoint pt1 = new GeoPoint((int) (latlng1[0] * 1E6),
					(int) (latlng1[1] * 1E6));
			GeoPoint pt2 = new GeoPoint((int) (latlng2[0] * 1E6),
					(int) (latlng2[1] * 1E6));
